            ...
            <div class="flex flex-col justify-center items-center md:items-start gap-2 mb-10 text-sm">
              ...
              {#each links as link}
                {#if link.navType == 'scroll'}
                    <ScrollLink class="link link-hover" whenUnselected="tab tab-sm tab-lifted flex-1" whenSelected="tab-active font-black !bg-base-100" href={link.pathname}>
                        {link.title}
                    </ScrollLink>
                {:else}
                    <PageLink class="link link-hover" whenUnselected="tab tab-sm tab-lifted flex-1" whenSelected="tab-active font-black !bg-base-100" href={link.pathname}>
                        {link.title}
                    </PageLink>
                {/if}
              {/each}
